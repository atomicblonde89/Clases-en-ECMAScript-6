...
formId.addEventListener("submit", e => {
  const data = [];
  const impuestoCliente = new Impuestos(montoBrutoAnualId.value, deduccionesId.value);
  const nuevoCliente = new Cliente(nombreClienteId.value, impuestoCliente);
  e.preventDefault();
  data.push(nuevoCliente);
 
  if (nombreClienteId.value.length > 0 && /^\s+$/.test(nombreClienteId.value) == false && montoBrutoAnualId.value >= 0 && deduccionesId.value >= 0) {
